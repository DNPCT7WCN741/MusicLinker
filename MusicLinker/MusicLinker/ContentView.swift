...
struct APISettingsSheet: View {
    ...
    @State private var isDeepseekEnabled: Bool = false
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section {
                    Toggle("启用 AI 歌词分析", isOn: $isDeepseekEnabled)
                    ...
                    if isDeepseekEnabled {
                        Text("用于提取歌词中最动人的 3 行和关键词。\n获取 API Key: https://platform.deepseek.com\n\n⚠️ 仅在浏览歌词时调用，不会自动消耗 token。")
                    } else {
                        Text("禁用后不会调用 AI，节省 token。歌词仍可通过热评或算法获取。")
                    }
                }
            }
            .navigationTitle("设置")
            ...
            .onAppear {
                ...
                // 从 Keychain 读取 API Key（安全存储）
                deepseekAPIKey = KeychainHelper.shared.retrieve(forKey: "DeepSeekAPIKey") ?? ""
                isDeepseekEnabled = UserDefaults.standard.bool(forKey: "IsDeepSeekEnabled")
            }
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("保存") {
                        ...
                        UserDefaults.standard.set(isDeepseekEnabled, forKey: "IsDeepSeekEnabled")
                        // 保存 API Key 到 Keychain（加密存储）
                        if !deepseekAPIKey.isEmpty {
                            try? KeychainHelper.shared.save(deepseekAPIKey, forKey: "DeepSeekAPIKey")
                        } else {
                            try? KeychainHelper.shared.delete(forKey: "DeepSeekAPIKey")
                        }
                        isPresented = false
                    }
                }
